...
- **File names by kind** (`nb_download_name()` in `lib/bootstrap.php`; one function drives the gallery, downloads, zips and admin):
  ...
  - **Videos (classmates') and PDFs**: keep their own uploaded name with spaces turned into dashes (`nb_dash_spaces()`: "Class Picnic 1986.mp4" -> "Class-Picnic-1986.mp4"), so a name never shows as `%20`. The original is stored untouched (`orig_name`). Videos still receive a number in the database (`seq`) but it is no longer used for their name. Two videos with the same name get `-2`, `-3` in a zip.
...
- Tests: `php tests/{bootstrap,credits,numbering,folders,derive,slideshow,errorlog,backup,thumbs}_test.php` (need GD, exiftool and ffmpeg). `bootstrap_test.php` checks that no core function has been accidentally deleted; run all of them after editing `lib/`.
