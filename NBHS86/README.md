...
- Passcode is matched exactly: case and spaces both count.
...
## Single-file download tries "Share to Photos" first
The lightbox's own download button (the one already there before this feature) now tries `navigator.share()` with the actual file before falling back to the plain attachment link. On a phone that supports it, this puts "Save to Photos" / "Save Video" directly in the OS share sheet, one tap, no long-press needed. Falls back to the old plain download link (unchanged) when: the browser doesn't support sharing files at all, the fetch or share fails for any reason (including the user cancelling), or the file is larger than `SHARE_MAX_BYTES` (25 MB) in `assets/js/gallery.js`, fetching a large video into memory before sharing is slow and risks Safari deciding the tap's momentary "user activation" expired before `navigator.share()` actually runs. Desktop and unsupported browsers see zero behavior change, the interception only fires when `navigator.canShare`/`navigator.share` exist. The lightbox's download button also shows a visible "Download" label next to the icon (`.pswp-dl-label`), not just the icon alone; its CSS needs an extra `.pswp__button` qualifier to out-specificity PhotoSwipe's own stylesheet, which loads after this one and otherwise pins the button to a fixed 50x50 icon-only size.

Also wired into "Download selected": ticking exactly one file shares/downloads it directly (same helper, same fallback), no zip at all; 2+ files still prepare-and-zip as before. The unzip-instructions line under the button (`#selZipHint`) starts `hidden` and is only shown by JS when 2+ files are selected, since a single-file download never produces a zip to unzip.
...
"Photos" folder view shows a one-line hint under the header, "Click on any photo to open it in a scrollable lightbox." Gated to `$folder['slug'] === 'photos'` specifically, not shown for Videos, Documents, Slideshows, or any custom folder.
...
- **File names by kind** (`nb_download_name()` in `lib/bootstrap.php`; one function drives the gallery, downloads, zips and admin):
  ...
  - **Slideshows**: keep their own file name (Bob names these himself before uploading), assigned only when the admin uploads them through the admin page's "Upload slideshows" box (the tus request carries `slideshow=1`, honoured only for a signed-in admin). They land in Slideshows credited to "Reunion Committee" and take no video number. `slideshow_seq` is still assigned internally (duplicate/backfill bookkeeping) but is not used in the file name.
